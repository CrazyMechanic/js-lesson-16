...
function getLocalDate(date, isSeconds, isISO) {
  ...
  if (isSeconds === false && isISO === true) {
    result = date.toLocaleString('ua', options).replace(/(\d{2}).(\d{2}).(\d{4})/, '$3.$2.$1');
    ...
    options.second = 'numeric';
    result = date.toLocaleString('ua', options);
  } else {
    result = date.toLocaleString('ua', options);
  }
...
function getWeekDay(date) {
  ...
  return new Intl.DateTimeFormat('ua', options).format(newDate);
}
...
function getDateAgo(date, days) {
  ...
  let newDays = 1000 * 60 * 60 * 24 * days;

  let someDate = newDate.getTime() - newDays;
...
let car = new Car(2000, 'Lacetti', 'Chevrolet', 2010);
let car2 = new Car(5000, 'FX50 AWD', 'Infinite', 2023);
